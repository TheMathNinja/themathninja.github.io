...
for index, year in enumerate(years):
    ...
    for record in year['rows']:
        rows.append(f'<tr data-custom="{esc(record["custom"])}"><td>{esc(record["player"])}</td><td><span class="position">{esc(record["custom"])}</span></td><td>{esc(record["default"])}</td>' + '</tr>')
    if y == '2026':
        description = f'<p class="snapshot">Updated {esc(year["asOf"])}</p>'
    else:
